...
namespace SPSA.Autorizadores.Infraestructura.Repositorio
{
    public class RepositorioMaeCaja : RepositorioGenerico<SGPContexto, Mae_Caja>, IRepositorioMaeCaja
    {
        ...
        public async Task<DataTable> ObtenerCajasPorLocalAsync(string codEmpresa, string codCadena, string codRegion, string codZona, string codLocal)
        {
            var query = @"
            SELECT 
                mc.""COD_EMPRESA"" || ' ' || me.""NOM_EMPRESA"" as ""EMPRESA"",
            ...
            INNER JOIN
                ...
                AND ml.""COD_ZONA"" = mz.""COD_ZONA""
            WHERE 
                    ml.""COD_EMPRESA"" = @CodEmpresa AND
                    ml.""COD_CADENA"" = @CodCadena AND
                    ml.""COD_REGION"" = @CodRegion AND
                    ml.""COD_ZONA"" = @CodZona AND
                    ml.""COD_LOCAL"" = @CodLocal
            ORDER BY
                mc.""COD_EMPRESA"",
                mc.""COD_CADENA"",
                ml.""COD_LOCAL"",
            ...
            using (var connection = new NpgsqlConnection(AppDBMyBDContext.Database.Connection.ConnectionString))
            {
                ...
                {
                    command.Parameters.AddWithValue("@CodEmpresa", codEmpresa);
                    command.Parameters.AddWithValue("@CodCadena", codCadena);
                    command.Parameters.AddWithValue("@CodRegion", codRegion);
