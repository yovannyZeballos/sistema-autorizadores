...
var urlListarSeriesDisponibles = baseUrl + 'Inventario/SeriesProducto/ListarPorProducto'; // o 'ListarDisponiblesPorProducto'
...
var AdministrarGuiaRecepcion = (function ($) {
    ...
    const eventos = function () {
        ...
        $('#btnNuevaGuia').on('click', async function () {
            limpiarModal();
            // cargar combos base para el modal (si aún no)
    ...
    function addDetalleRow() {
        ...
        const tr = $(
            ...
            /*'<td><input type="text" class="form-control form-control-sm inpObs" /></td>' +*/
    ...
    async function guardarRecepcion() {
        ...
        if (!header.NumGuia || !header.Fecha || !header.AreaGestion || !header.ClaseStock || !header.EstadoStock) {
    ...
    function limpiarModal() {
        ...
        $('#recEmpDest').val(null).trigger('change');
        $('#recLocDest').empty().append('<option></option>').val(null).trigger('change');
    ...
    return {
        init: function () {
            checkSession(async function () {
                eventos();
                // combos de filtros (si los usas en el index)
                if ($('#cboProveedor').length) {
                    $('#cboProveedor').select2({ width: '100%', placeholder: 'Todos' });
                }
                visualizarDataTable();
            });
        }
